...
function App() {
  ...
  const handleDeleteJobs = (id) => {
    const localStorageJob = JSON.parse(localStorage.getItem("jobs")); // lấy ra item từ local
    const newJobs = localStorageJob.filter((item, index) => index !== id);// lọc ra những thằng có id khác với id đang được bấm vào nút xóa
      ...
      <ul>
        {jobs.map((job, index) => (
          <li key={index}>
            {job}
            <button
              onClick={() => handleDeleteJobs(index)}
              style={{ marginLeft: 10 }}
            >
              xóa
            </button>
